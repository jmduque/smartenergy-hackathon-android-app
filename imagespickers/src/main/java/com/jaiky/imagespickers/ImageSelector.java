...
public class ImageSelector {
    ...
    public static void open(Activity activity, ImageConfig config) {
        ...
        if (config.getImageLoader() == null) {
            Toast.makeText(activity, R.string.imagepicker_open_camera_fail, Toast.LENGTH_SHORT).show();
            return;
        }

        if (!Utils.existSDCard()) {
            Toast.makeText(activity, R.string.imagepicker_empty_sdcard, Toast.LENGTH_SHORT).show();
            return;
        }
    ...
    public static void open(Fragment fragment, ImageConfig config) {
        ...
        if (config.getImageLoader() == null) {
            Toast.makeText(fragment.getActivity(), R.string.imagepicker_open_camera_fail, Toast.LENGTH_SHORT).show();
            return;
        }

        if (!Utils.existSDCard()) {
            Toast.makeText(fragment.getActivity(), R.string.imagepicker_empty_sdcard, Toast.LENGTH_SHORT).show();
            return;
        }
