 ...
 */
interface ChangePaymentPasswordRequest {

    @PUT("payment/password")
    fun changePassword(@Body body: ChangePasswordModel): Call<AuthModel>

    class Builder(context: Context) : RequestBuilderWithBody<ChangePasswordModel, AuthModel>(context) {
        ...
        override fun request() {
            val serviceRequest = EnergoRetrofitCallBuilder.createService(
                    context,
                    ChangePaymentPasswordRequest::class.java
            )
    ...
    interface ChangePaymentPasswordResultListener : BaseResultListener<AuthModel>
