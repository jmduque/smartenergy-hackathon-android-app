package com.energolabs.evergo.modules.register.fragments
...
import com.energolabs.evergo.modules.register.requests.RegisterByPhoneRequest
...
class RegisterFragment : BaseFragment(), View.OnClickListener {
    ...
    // ERROR VIEWS
    private var tv_register_error: TextView? = null
    ...
    override fun findViews(view: View) {
        ...
        // ERROR VIEWS
        tv_register_error = view.findViewById(R.id.tv_register_error) as TextView
    ...
    private fun requestRegister() {
        ...
        if (TextUtils.isEmpty(phoneCode)) {
            et_phone_code?.setText(R.string.energo_phone_code_error)
            return
        }
        ...
        RegisterByPhoneRequest
                ...
                .setResultListener(registerByPhoneResultListener)
                .request()
    }

    private val registerByPhoneResultListener = object : RegisterByPhoneRequest.RegisterByPhoneResultListener {
        override fun onResultSuccess(
                tag: Any?,
                response: AuthModel?
        ) {
            hideWaitDialog()
            AuthPreferences(activity ?: return).saveAuthModel(
                    response
            )
            startActivity(
                    MainActivity.makeIntent(activity ?: return)
            )
            activity?.setResult(REGISTER_OK)
            activity?.finish()
        }
        ...
        override fun onResultError(
            ...
            hideWaitDialog()
            tv_register_error?.text = error
        }
    }

    override fun onBackPressed(): Boolean {
        activity?.setResult(REGISTER_KO)
        return super.onBackPressed()
    }

    companion object {
        val REGISTER = 2000
        val REGISTER_OK = 2001
        val REGISTER_KO = 2002
