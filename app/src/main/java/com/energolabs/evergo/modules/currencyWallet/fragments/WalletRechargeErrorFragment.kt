...
class WalletRechargeErrorFragment : BaseFragment(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        val args = arguments
        error = args?.getString(ERROR_MESSAGE)
        super.onCreate(savedInstanceState)
    }

    override val layoutId: Int
        get() = R.layout.fragment_wallet_recharge_error
    ...
    private fun updateErrorMessage() {
        if (TextUtils.isEmpty(error)) {
            tv_error_message?.setText(R.string.energo_wallet_recharge_error_message)
        } else {
